...
export default function PurchaseOrderPanel({
  ...
  // Celebration state for purchase orders
  ...
  // purchase logs for tracking operation list
  const [purchaseLogs, setPurchaseLogs] = useState<Array<{
    id: string;
    materialKey: string;
    nameText: string;
    qty: number;
    ...
    try {
      const saved = localStorage.getItem('albion_purchase_logs');
    ...
    pendingJobs.forEach(job => {
      ...
      if (mp.lingotes.needed > 0) {
        ...
        if (remaining > 0) {
          if (!map[key]) {
            map[key] = {
              key, type: 'lingotes', nameText: `Lingote ${job.tier}.${job.enchantment}`,
      ...
      if (mp.tablas.needed > 0) {
        ...
        if (remaining > 0) {
          if (!map[key]) {
            map[key] = {
              key, type: 'tablas', nameText: `Tabla ${job.tier}.${job.enchantment}`,
              tier: job.tier, enchantment: job.enchantment, artifactName: '',
              totalNeeded: 0, totalAcquired: 0
            };
          }
      ...
      if (mp.telas.needed > 0) {
        ...
        if (remaining > 0) {
          if (!map[key]) {
            map[key] = {
              key, type: 'telas', nameText: `Tela ${job.tier}.${job.enchantment}`,
              tier: job.tier, enchantment: job.enchantment, artifactName: '',
              totalNeeded: 0, totalAcquired: 0
            };
          }
      ...
      if (mp.cueros.needed > 0) {
        ...
        if (remaining > 0) {
          if (!map[key]) {
            map[key] = {
              key, type: 'cueros', nameText: `Cuero ${job.tier}.${job.enchantment}`,
              tier: job.tier, enchantment: job.enchantment, artifactName: '',
              totalNeeded: 0, totalAcquired: 0
            };
          }
      ...
      if (mp.artefacto.needed > 0) {
        ...
        if (remaining > 0) {
          if (!map[key]) {
            map[key] = {
              key, type: 'artefacto', nameText: `${job.recipe.Artefacto1} ${job.tier}`,
  ...
  const handleCompleteManualBuy = (mat: StackedMaterial) => {
    ...
    setPurchaseLogs(prev => {
      ...
      localStorage.setItem('albion_purchase_logs', JSON.stringify(updated));
      return updated;
    });
    ...
    // Trigger striking celebration popup for buy order
    setCelebration({
      show: true,
      itemName: mat.nameText,
      qty: actualBuy,
      cost: totalCost
    });
  ...
  return (
    <div className="max-w-6xl mx-auto py-2">
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  {/* Material specific operations logs */}
                  {(() => {
                    const matLogs = purchaseLogs.filter(log => log.materialKey === mat.key);
                    if (matLogs.length === 0) return null;
                    return (
                      <div className="mt-3 pt-3 border-t border-gray-800 text-[10px]">
                        <span className="text-gray-400 font-semibold block mb-1">Operaciones de esta tanda:</span>
                        <div className="max-h-24 overflow-y-auto space-y-1 pr-1">
                          {matLogs.map(log => (
                            <div key={log.id} className="flex justify-between items-center bg-black/30 px-2 py-1 rounded border border-gray-850 font-mono">
                              <span className="text-emerald-400 font-bold">-{log.qty} ud</span>
                              <span className="text-gray-400">-{log.cost.toLocaleString()} s</span>
                ...
                {(() => {
                  const manualVal = manualPurchaseInputs[mat.key];
                  const declaredQty = manualVal !== undefined && manualVal !== 0 ? manualVal : mat.totalNeeded;
                  const reachesZero = declaredQty >= mat.totalNeeded;
                  const btnClass = reachesZero 
                    ? 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-extrabold border border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.25)]' 
                    : primaryColor;
                  ...
                  return (
                    <div className="mt-4 pt-4 border-t border-gray-800/80 flex items-center justify-between gap-3">
                      ...
                      >
                        <Check size={14} />
                        {reachesZero ? 'Completar Compra (0)' : 'Compra Realizada'}
                      </button>
                    </div>
                  );
                })()}
      ...
      {/* Global Purchase Logs History Section */}
      {purchaseLogs.length > 0 && (
        <div className={`mt-8 p-5 rounded-xl border bg-black/40 ${borderColor}`}>
          <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-3">
            <h3 className="font-semibold text-white font-serif flex items-center gap-2 text-sm">
            ...
            <button
              onClick={() => {
                setPurchaseLogs([]);
                localStorage.removeItem('albion_purchase_logs');
              }}
              className="px-2.5 py-1 text-[10px] uppercase font-bold text-red-400 bg-red-500/10 hover:bg-red-500/20 rounded border border-red-500/20 cursor-pointer transition-all"
            >
              Limpiar Historial
            </button>
          </div>
          ...
          <div className="max-h-60 overflow-y-auto space-y-2 pr-1 font-mono text-xs">
            {purchaseLogs.map((log) => (
              <div 
                key={log.id} 
                className="flex flex-col sm:flex-row sm:items-center justify-between p-2.5 bg-black/20 rounded border border-gray-850 gap-2 hover:bg-black/30 transition-all"
              >
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold bg-neutral-800 text-amber-300 px-1.5 py-0.5 rounded border border-gray-700">
                    {log.timestamp}
                  </span>
                  <span className="text-white font-serif font-bold">{log.nameText}</span>
                  <span className="text-gray-500 text-[10px]">({log.city} - {log.method === 'direct' ? 'Directo' : 'Orden'})</span>
                </div>
                <div className="flex items-center gap-4 text-right">
                  <div>
                    <span className="text-gray-500 block text-[9px] uppercase">Comprado:</span>
                    <strong className="text-emerald-400 font-bold">-{log.qty} unidades</strong>
                  </div>
                  <div>
                    <span className="text-gray-500 block text-[9px] uppercase">Costo Total:</span>
                    <strong className="text-red-400">{log.cost.toLocaleString()} s</strong>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* CELEBRATION MODAL OVERLAY FOR BUY ORDER */}
      {celebration && (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm z-50 animate-fadeIn">
          <div className="bg-slate-900 border-2 border-amber-500/50 rounded-2xl p-6 max-w-sm w-full text-center space-y-4 shadow-[0_0_50px_rgba(245,158,11,0.35)] transform scale-105 transition-all">
            <div className="w-16 h-16 bg-amber-500/20 text-amber-400 rounded-full flex items-center justify-center mx-auto text-3xl border-2 border-amber-400/30 animate-bounce">
              🛒
            </div>
            <div className="space-y-1">
              <h4 className="text-amber-400 font-extrabold font-mono text-xs uppercase tracking-widest">¡Compra Exitosa!</h4>
              <p className="text-white text-md font-bold font-serif leading-snug">
                Recurso <span className="text-amber-300">"{celebration.itemName}" x{celebration.qty}</span> comprado de forma exitosa.
              </p>
            </div>
            <div className="bg-slate-950 py-3.5 px-4 rounded-xl border border-slate-850/80">
              <span className="block text-[10px] text-slate-500 font-mono uppercase tracking-wider">Costo de Inversión</span>
              <span className="text-xl font-black text-red-400 font-mono tracking-wide">-{celebration.cost.toLocaleString()} S</span>
            </div>
            <button
              onClick={() => setCelebration(null)}
              className="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold rounded-xl transition-colors font-mono uppercase tracking-wider shadow-[0_4px_12px_rgba(245,158,11,0.2)]"
            >
              ¡Excelente!
            </button>
          </div>
