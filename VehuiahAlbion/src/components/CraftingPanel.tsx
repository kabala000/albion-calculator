...
interface CraftingPanelProps {
  ...
  onModifyInventoryQuantity: (key: string, change: number) => void;
}
...
export default function CraftingPanel({
  ...
  // Reserves toggling states
  const [useReserveLingotes, setUseReserveLingotes] = useState<boolean>(false);
  const [useReserveTablas, setUseReserveTablas] = useState<boolean>(false);
  const [useReserveTelas, setUseReserveTelas] = useState<boolean>(false);
  const [useReserveCueros, setUseReserveCueros] = useState<boolean>(false);
  const [useReserveArtifact, setUseReserveArtifact] = useState<boolean>(false);

  // Journal Configuration States
  const [useDiarios, setUseDiarios] = useState<boolean>(true);
  const [useReserveJournals, setUseReserveJournals] = useState<boolean>(false);
  const [buyJournals, setBuyJournals] = useState<boolean>(false);
  const [journalsBuyMethod, setJournalsBuyMethod] = useState<'direct' | 'order'>('order');
  const [manualJournalPriceText, setManualJournalPriceText] = useState<string>('2000');

  // Reset reserves states on configuration shifts to prevent stale carryovers
  React.useEffect(() => {
    setUseReserveLingotes(false);
    setUseReserveTablas(false);
    setUseReserveTelas(false);
    setUseReserveCueros(false);
    setUseReserveArtifact(false);
    setUseReserveJournals(false);
    setBuyJournals(false);
  }, [selectedRecipeIndex, selectedTier, selectedEnchantment, craftQuantity]);

  // Adjust default journal price based on Tier
  React.useEffect(() => {
    const cleanNum = String(selectedTier).replace(/\D/g, '');
    switch (cleanNum) {
      case '4': setManualJournalPriceText('1000'); break;
      case '5': setManualJournalPriceText('2000'); break;
      case '6': setManualJournalPriceText('4500'); break;
      case '7': setManualJournalPriceText('12000'); break;
      case '8': setManualJournalPriceText('35000'); break;
      default: setManualJournalPriceText('2000'); break;
    }
  }, [selectedTier]);
  ...
  // 5.5 Reserves stock & coverage math
  const stockLingotes = useMemo(() => inventory[`lingotes_${selectedTier}_${selectedEnchantment}`] || 0, [inventory, selectedTier, selectedEnchantment]);
  const stockTablas = useMemo(() => inventory[`tablas_${selectedTier}_${selectedEnchantment}`] || 0, [inventory, selectedTier, selectedEnchantment]);
  const stockTelas = useMemo(() => inventory[`telas_${selectedTier}_${selectedEnchantment}`] || 0, [inventory, selectedTier, selectedEnchantment]);
  const stockCueros = useMemo(() => inventory[`cueros_${selectedTier}_${selectedEnchantment}`] || 0, [inventory, selectedTier, selectedEnchantment]);
  const stockArtifact = useMemo(() => recipe.Artefacto1 ? (inventory[`artefacto_${recipe.Artefacto1}_${selectedTier}`] || 0) : 0, [inventory, recipe.Artefacto1, selectedTier]);

  const unitsCoveredLingotes = useMemo(() => useReserveLingotes ? Math.min(materialsBreakdown.lingotes.final, stockLingotes) : 0, [useReserveLingotes, materialsBreakdown.lingotes.final, stockLingotes]);
  const unitsCoveredTablas = useMemo(() => useReserveTablas ? Math.min(materialsBreakdown.tablas.final, stockTablas) : 0, [useReserveTablas, materialsBreakdown.tablas.final, stockTablas]);
  const unitsCoveredTelas = useMemo(() => useReserveTelas ? Math.min(materialsBreakdown.telas.final, stockTelas) : 0, [useReserveTelas, materialsBreakdown.telas.final, stockTelas]);
  const unitsCoveredCueros = useMemo(() => useReserveCueros ? Math.min(materialsBreakdown.cueros.final, stockCueros) : 0, [useReserveCueros, materialsBreakdown.cueros.final, stockCueros]);
  const unitsCoveredArtifact = useMemo(() => useReserveArtifact ? Math.min(materialsBreakdown.artifact.needed, stockArtifact) : 0, [useReserveArtifact, materialsBreakdown.artifact.needed, stockArtifact]);

  const discountedMaterialsCost = useMemo(() => {
    const pLingote = parseInt(manualPricesText.lingotes) || 0;
    const pTabla = parseInt(manualPricesText.tablas) || 0;
    const pTela = parseInt(manualPricesText.telas) || 0;
    const pCuero = parseInt(manualPricesText.cueros) || 0;
    ...
    const remainingLingotes = Math.max(0, materialsBreakdown.lingotes.final - unitsCoveredLingotes);
    const remainingTablas = Math.max(0, materialsBreakdown.tablas.final - unitsCoveredTablas);
    const remainingTelas = Math.max(0, materialsBreakdown.telas.final - unitsCoveredTelas);
    const remainingCueros = Math.max(0, materialsBreakdown.cueros.final - unitsCoveredCueros);
    const remainingArtifact = Math.max(0, materialsBreakdown.artifact.needed - unitsCoveredArtifact);

    return (remainingLingotes * pLingote) + (remainingTablas * pTabla) + (remainingTelas * pTela) + (remainingCueros * pCuero) + (remainingArtifact * pArtifact);
  }, [materialsBreakdown, unitsCoveredLingotes, unitsCoveredTablas, unitsCoveredTelas, unitsCoveredCueros, unitsCoveredArtifact, manualPricesText]);
  ...
  const professionId = useMemo(() => {
    return activeArtisan === 'Flechero' ? 'FLETCHER' : activeArtisan === 'Herrero Mágico' ? 'MAGE' : 'BLACKSMITH';
  }, [activeArtisan]);

  const journalKey = useMemo(() => {
    return `diarios_EMPTY_${selectedTier}_${professionId}`;
  }, [selectedTier, professionId]);

  const stockJournals = useMemo(() => inventory[journalKey] || 0, [inventory, journalKey]);

  const requiredJournals = useMemo(() => {
    return journalsMath.totalBooksNeeded;
  }, [journalsMath.totalBooksNeeded]);

  const unitsCoveredJournals = useMemo(() => {
    return useReserveJournals ? Math.min(requiredJournals, stockJournals) : 0;
  }, [useReserveJournals, requiredJournals, stockJournals]);

  const remainingJournalsToBuy = useMemo(() => {
    return Math.max(0, requiredJournals - unitsCoveredJournals);
  }, [requiredJournals, unitsCoveredJournals]);

  const journalsPurchasedCost = useMemo(() => {
    if (!useDiarios) return 0;
    const price = parseInt(manualJournalPriceText) || 0;
    const qty = buyJournals ? remainingJournalsToBuy : 0;
    return Math.round(qty * price * (1 + (journalsBuyMethod === 'order' ? 0.025 : 0)));
  }, [useDiarios, buyJournals, remainingJournalsToBuy, manualJournalPriceText, journalsBuyMethod]);
    ...
    const financials = useMemo(() => {
      ...
      // Use discountedMaterialsCost to calculate real investment cost (discounting reserves)
      const totalInvestment = discountedMaterialsCost + stationFeeMath.totalSilverCost + setupFeeValue + (useDiarios ? journalsPurchasedCost : 0);
    ...
    }, [manualUnitSellPriceText, craftQuantity, sellMethod, globalPremium, discountedMaterialsCost, stationFeeMath.totalSilverCost, useDiarios, journalsPurchasedCost]);

  // Handle Add to Queue trigger
  const handleAddToQueue = () => {
    // 1. If any materials or artifacts are covered by local reserves, subtract them from the character's inventory
    if (useReserveLingotes && unitsCoveredLingotes > 0) {
      onModifyInventoryQuantity(`lingotes_${selectedTier}_${selectedEnchantment}`, -unitsCoveredLingotes);
    }
    if (useReserveTablas && unitsCoveredTablas > 0) {
      onModifyInventoryQuantity(`tablas_${selectedTier}_${selectedEnchantment}`, -unitsCoveredTablas);
    }
    if (useReserveTelas && unitsCoveredTelas > 0) {
      onModifyInventoryQuantity(`telas_${selectedTier}_${selectedEnchantment}`, -unitsCoveredTelas);
    }
    if (useReserveCueros && unitsCoveredCueros > 0) {
      onModifyInventoryQuantity(`cueros_${selectedTier}_${selectedEnchantment}`, -unitsCoveredCueros);
    }
    if (useReserveArtifact && unitsCoveredArtifact > 0 && recipe.Artefacto1) {
      onModifyInventoryQuantity(`artefacto_${recipe.Artefacto1}_${selectedTier}`, -unitsCoveredArtifact);
    }
    // Deduct empty journals if configured and covered
    if (useDiarios && useReserveJournals && unitsCoveredJournals > 0) {
      onModifyInventoryQuantity(journalKey, -unitsCoveredJournals);
    }

    const hasLingotesLeft = materialsBreakdown.lingotes.final > unitsCoveredLingotes;
    const hasTablasLeft = materialsBreakdown.tablas.final > unitsCoveredTablas;
    const hasTelasLeft = materialsBreakdown.telas.final > unitsCoveredTelas;
    const hasCuerosLeft = materialsBreakdown.cueros.final > unitsCoveredCueros;
    const hasArtifactLeft = materialsBreakdown.artifact.needed > unitsCoveredArtifact;

    const materialsReady = !hasLingotesLeft && !hasTablasLeft && !hasTelasLeft && !hasCuerosLeft && !hasArtifactLeft;
    const journalsReady = !useDiarios || (unitsCoveredJournals + (buyJournals ? remainingJournalsToBuy : 0) >= requiredJournals);
    const allReady = materialsReady && journalsReady;
    ...
    const job: CraftingJob = {
      ...
      materialsCost: discountedMaterialsCost,
      ...
      materialsPurchased: {
        lingotes: { needed: materialsBreakdown.lingotes.final, purchased: unitsCoveredLingotes, method: lingotesMethod, city: lingotesCity },
        tablas: { needed: materialsBreakdown.tablas.final, purchased: unitsCoveredTablas, method: tablasMethod, city: tablasCity },
        telas: { needed: materialsBreakdown.telas.final, purchased: unitsCoveredTelas, method: telasMethod, city: telasCity },
        cueros: { needed: materialsBreakdown.cueros.final, purchased: unitsCoveredCueros, method: cuerosMethod, city: cuerosCity },
        artefacto: { needed: materialsBreakdown.artifact.needed, purchased: unitsCoveredArtifact, method: artifactMethod, city: artifactCity }
      },
      materialsReady,
      status: allReady ? 'ready_to_craft' : 'pending_materials',

      // Journal configuration fields
      useDiarios,
      diariosQty: requiredJournals,
      diariosTier: selectedTier,
      diariosProfession: professionId,
      diariosCovered: unitsCoveredJournals,
      diariosPurchased: buyJournals ? remainingJournalsToBuy : 0,
      diariosBuyMethod: journalsBuyMethod,
      diariosPrice: parseInt(manualJournalPriceText) || 0,
      diariosReady: journalsReady
    };

    onAddJob(job);
    
    // Reset our reserves checked options
    setUseReserveLingotes(false);
    setUseReserveTablas(false);
    setUseReserveTelas(false);
    setUseReserveCueros(false);
      ...
      <div className="xl:col-span-8 space-y-6">
        ...
        <div className={`p-5 rounded-xl border ${surfaceColor} ${borderColor}`}>
          <div className="flex items-center justify-between mb-4 border-b border-gray-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-white">(Configurar Item)</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Checkbox Usar Diarios */}
            <div className="flex items-center gap-3 md:col-span-2 bg-black/45 p-3 rounded-xl border border-gray-800/85 mt-2 shadow-inner">
              <label className="flex items-center gap-2 text-xs font-mono text-slate-300 cursor-pointer select-none">
                <input 
                  type="checkbox"
                  checked={useDiarios}
                  onChange={(e) => setUseDiarios(e.target.checked)}
                  className="rounded border-gray-700 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-4 h-4 cursor-pointer"
                />
                <span className="text-amber-400 font-bold uppercase tracking-wider">Usar Diarios</span>
              </label>
              <span className="text-[10px] text-gray-400 leading-none">
                (Si está activado, se descontarán o comprarán diarios vacíos y se estimará la ganancia con diarios llenos)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className={`p-5 rounded-xl border ${surfaceColor} ${borderColor} relative overflow-hidden`}>
          ...
          <div className="flex items-center gap-2 mb-4 border-b border-gray-800 pb-3">
            ...
            <h3 className="font-semibold text-white">{useDiarios ? 'Diarios y Fama' : 'Fama'}</h3>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            {/* Fame Column */}
            <div className={`${useDiarios ? 'lg:col-span-2' : 'lg:col-span-5'} p-4 bg-gradient-to-br from-black/50 to-black/30 rounded-xl flex items-center gap-4 border border-gray-800 shadow-inner group`}>
            ...
            {/* Manufacturer Journals Setup - Only show if useDiarios is checked */}
            {useDiarios && (
        ...
        <div className={`p-5 rounded-xl border ${surfaceColor} ${borderColor}`}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3.5">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-black/40 border border-gray-700 text-white rounded px-2 py-1 text-[11px] focus:ring-1 focus:ring-amber-500 focus:outline-none mb-1.5"
                      >
                        {ALBION_CITIES.map((c) => (
                      ...
                      {/* Reservas info under select */}
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-amber-500/80 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockLingotes > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockLingotes} u</strong></span>
                        </div>
                        {stockLingotes > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveLingotes}
                              onChange={(e) => setUseReserveLingotes(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(materialsBreakdown.lingotes.final, stockLingotes)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                    ...
                    {materialsBreakdown.lingotes.final > 0 && (
                      <div className="mt-1.5 pt-1.5 border-t border-gray-850 text-[10px] font-mono leading-tight space-y-0.5">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({(materialsBreakdown.lingotes.final - unitsCoveredLingotes)} u):</span>
                          <span>{((materialsBreakdown.lingotes.final - unitsCoveredLingotes) * (parseInt(manualPricesText.lingotes) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{Math.round(
                            (materialsBreakdown.lingotes.final - unitsCoveredLingotes) * 
                ...
                <div className="p-3 bg-black/40 rounded-lg border border-gray-800 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      ...
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-amber-500/80 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockTablas > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockTablas} u</strong></span>
                        </div>
                        {stockTablas > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveTablas}
                              onChange={(e) => setUseReserveTablas(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(materialsBreakdown.tablas.final, stockTablas)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    {materialsBreakdown.tablas.final > 0 && (
                      <div className="mt-1.5 pt-1.5 border-t border-gray-850 text-[10px] font-mono leading-tight space-y-0.5">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({(materialsBreakdown.tablas.final - unitsCoveredTablas)} u):</span>
                          <span>{((materialsBreakdown.tablas.final - unitsCoveredTablas) * (parseInt(manualPricesText.tablas) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{Math.round(
                            (materialsBreakdown.tablas.final - unitsCoveredTablas) * 
                ...
                <div className="p-3 bg-black/40 rounded-lg border border-gray-800 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      ...
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-amber-500/80 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockTelas > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockTelas} u</strong></span>
                        </div>
                        {stockTelas > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveTelas}
                              onChange={(e) => setUseReserveTelas(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(materialsBreakdown.telas.final, stockTelas)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    {materialsBreakdown.telas.final > 0 && (
                      <div className="mt-1.5 pt-1.5 border-t border-gray-850 text-[10px] font-mono leading-tight space-y-0.5">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({(materialsBreakdown.telas.final - unitsCoveredTelas)} u):</span>
                          <span>{((materialsBreakdown.telas.final - unitsCoveredTelas) * (parseInt(manualPricesText.telas) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{Math.round(
                            (materialsBreakdown.telas.final - unitsCoveredTelas) * 
                ...
                <div className="p-3 bg-black/40 rounded-lg border border-gray-800 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      ...
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-amber-500/80 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockCueros > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockCueros} u</strong></span>
                        </div>
                        {stockCueros > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveCueros}
                              onChange={(e) => setUseReserveCueros(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(materialsBreakdown.cueros.final, stockCueros)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    {materialsBreakdown.cueros.final > 0 && (
                      <div className="mt-1.5 pt-1.5 border-t border-gray-850 text-[10px] font-mono leading-tight space-y-0.5">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({(materialsBreakdown.cueros.final - unitsCoveredCueros)} u):</span>
                          <span>{((materialsBreakdown.cueros.final - unitsCoveredCueros) * (parseInt(manualPricesText.cueros) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{Math.round(
                            (materialsBreakdown.cueros.final - unitsCoveredCueros) * 
                ...
                <div className="p-3 bg-amber-500/5 rounded-lg border border-amber-500/20 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-black/40 border border-amber-500/30 text-white rounded px-2 py-1 text-[11px] focus:ring-1 focus:ring-amber-500 focus:outline-none mb-1.5"
                      >
                        {ALBION_CITIES.map((c) => (
                          <option key={c} value={c} className="bg-neutral-900">{c}</option>
                        ))}
                      ...
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-amber-500/80 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockArtifact > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockArtifact} u</strong></span>
                        </div>
                        {stockArtifact > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveArtifact}
                              onChange={(e) => setUseReserveArtifact(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(materialsBreakdown.artifact.needed, stockArtifact)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    {materialsBreakdown.artifact.needed > 0 && (
                      <div className="mt-1.5 pt-1.5 border-t border-gray-850 text-[10px] font-mono leading-tight space-y-0.5">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({(materialsBreakdown.artifact.needed - unitsCoveredArtifact)} u):</span>
                          <span>{((materialsBreakdown.artifact.needed - unitsCoveredArtifact) * (parseInt(manualPricesText.artefacto) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{Math.round(
                            (materialsBreakdown.artifact.needed - unitsCoveredArtifact) * 
              ...
              {/* 3. DIARIOS VACÍOS */}
              {useDiarios && (
                <div className="p-3 bg-black/40 rounded-lg border border-blue-900/30 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-7 h-7 rounded bg-black/55 border border-gray-800 overflow-hidden flex items-center justify-center shrink-0 shadow-inner">
                        <img 
                          src={`https://render.albiononline.com/v1/item/${selectedTier}_JOURNAL_${professionId}.png`}
                          alt="Diario Vacío" 
                          className="w-6 h-6 object-contain"
                          referrerPolicy="no-referrer"
                          onError={handleImageLoadError}
                      ...
                      <strong className="text-xs text-white truncate">Diario Vacío T{selectedTier.substring(1)}</strong>
                    </div>
                    <span className="text-[10px] text-gray-400 font-mono shrink-0">Req: {requiredJournals} u</span>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    <div>
                      {/* Reservas info */}
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight h-full justify-center">
                        <div className="flex items-center gap-1 text-gray-400">
                          <Archive size={10} className="text-blue-400 shrink-0" />
                          <span className="truncate">Reserva: <strong className={stockJournals > 0 ? "text-emerald-400 font-bold" : "text-gray-500"}>{stockJournals} u</strong></span>
                        </div>
                        {stockJournals > 0 && (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none mt-1">
                            <input 
                              type="checkbox" 
                              checked={useReserveJournals}
                              onChange={(e) => setUseReserveJournals(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3 cursor-pointer"
                            />
                            <span>Descontar: <strong className="text-white">-{Math.min(requiredJournals, stockJournals)}</strong></span>
                          </label>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      {/* Check comprar */}
                      <div className="flex flex-col bg-black/30 p-1.5 rounded border border-gray-800/60 text-[9px] font-mono leading-tight h-full justify-center">
                        {remainingJournalsToBuy > 0 ? (
                          <label className="flex items-center gap-1 text-amber-500 cursor-pointer select-none">
                            <input 
                              type="checkbox" 
                              checked={buyJournals}
                              onChange={(e) => setBuyJournals(e.target.checked)}
                              className="rounded border-gray-750 bg-neutral-950 text-amber-500 focus:ring-0 focus:ring-offset-0 w-3 h-3 cursor-pointer"
                            />
                            <span className="truncate">Comprar: <strong className="text-white">{remainingJournalsToBuy} u</strong></span>
                          </label>
                        ) : (
                          <span className="text-gray-500 italic">Reserva cubre todo</span>
                        )}

                        {buyJournals && remainingJournalsToBuy > 0 && (
                          <div className="grid grid-cols-2 gap-1 mt-1">
                            <button
                              type="button"
                              onClick={() => setJournalsBuyMethod('order')}
                              className={`py-0.5 rounded text-[8px] font-bold cursor-pointer ${journalsBuyMethod === 'order' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-black/30 text-gray-500 border border-gray-800'}`}
                            >
                              Orden
                            </button>
                            <button
                              type="button"
                              onClick={() => setJournalsBuyMethod('direct')}
                              className={`py-0.5 rounded text-[8px] font-bold cursor-pointer ${journalsBuyMethod === 'direct' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-black/30 text-gray-500 border border-gray-800'}`}
                            >
                              Directo
                            </button>
                          </div>
                  ...
                  {buyJournals && remainingJournalsToBuy > 0 && (
                    <div className="space-y-1 mt-1 pt-1 border-t border-gray-800/50">
                      <div>
                        <label className="text-[9px] text-gray-500 uppercase tracking-wider block">Precio Compra Diario s (Silver)</label>
                        <input
                          type="number"
                          min="0"
                          value={manualJournalPriceText}
                          onChange={(e) => setManualJournalPriceText(e.target.value)}
                          className="w-full bg-black/45 border border-gray-750 text-white font-mono rounded px-2 py-0.5 text-[10px] focus:ring-1 focus:ring-amber-500 focus:outline-none"
                          placeholder="Precio s..."
                        />
                      </div>
                      <div className="text-[10px] font-mono leading-tight pt-1">
                        <div className="flex justify-between text-gray-400">
                          <span>Subtotal Base ({remainingJournalsToBuy} u):</span>
                          <span>{(remainingJournalsToBuy * (parseInt(manualJournalPriceText) || 0)).toLocaleString()} s</span>
                        </div>
                        <div className="flex justify-between text-amber-400 font-semibold">
                          <span>Total con Impuesto:</span>
                          <span>{journalsPurchasedCost.toLocaleString()} s</span>
                        </div>
                      </div>
                    </div>
                  )}
        ...
        <div className={`p-5 rounded-xl border bg-black/40 ${borderColor}`}>
          ...
          <div className="space-y-2.5 font-mono text-xs text-gray-300">
            <div className="flex justify-between border-b border-gray-800 pb-1">
              <span>Costo Teórico Materiales:</span>
              <strong className="text-gray-400 line-through">{materialsBreakdown.totalRawCost.toLocaleString()} s</strong>
            </div>
            
            {materialsBreakdown.totalRawCost - discountedMaterialsCost > 0 && (
              <div className="flex justify-between border-b border-gray-800 pb-1 text-emerald-400">
                <span>Descuento de Reservas:</span>
                <strong>-{(materialsBreakdown.totalRawCost - discountedMaterialsCost).toLocaleString()} s</strong>
              </div>
            )}

            <div className="flex justify-between border-b border-gray-800 pb-1">
              <span>Costo Real Materiales:</span>
              <strong className="text-amber-400 font-bold">{discountedMaterialsCost.toLocaleString()} s</strong>
            </div>
            ...
            {useDiarios && (
              <div className="flex justify-between border-b border-gray-800 pb-1.5 flex-col pb-2 text-gray-300">
                <div className="flex justify-between">
                  <span>Inversión de Diarios:</span>
                  <strong className="text-white">
                    {useReserveJournals && unitsCoveredJournals > 0 && buyJournals && remainingJournalsToBuy > 0 ? (
                      `${journalsPurchasedCost.toLocaleString()} s`
                    ) : useReserveJournals && unitsCoveredJournals > 0 ? (
                      '0 s'
                    ) : buyJournals && remainingJournalsToBuy > 0 ? (
                      `${journalsPurchasedCost.toLocaleString()} s`
                    ) : (
                ...
                <span className="text-[10px] text-gray-500 italic mt-0.5 leading-tight">
                  {useReserveJournals && unitsCoveredJournals > 0 && buyJournals && remainingJournalsToBuy > 0 ? (
                    `Mixto: Almacén ${unitsCoveredJournals} u (0 s) + Comprado ${remainingJournalsToBuy} u`
                  ) : useReserveJournals && unitsCoveredJournals > 0 ? (
                    `Almacén: ${unitsCoveredJournals} u usadas de reservas`
                  ) : buyJournals && remainingJournalsToBuy > 0 ? (
                    `Comprado: ${remainingJournalsToBuy} u compradas a mercado`
                  ) : (
                    'Omitido o pendiente de asignar compra/reserva'
                  )}
                </span>
              </div>
            )}
