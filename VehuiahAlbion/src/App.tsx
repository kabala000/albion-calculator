...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Audio synchronization failed/not supported:", e);
    }
  ...
  // Derived state: Active queues/jobs estimated profit
  const activeJobs = queues[activeCharName]?.filter(j => j.status !== 'completed') || [];
  const totalActiveProfit = activeJobs.reduce((sum, job) => sum + ((job.netSellRevenue || 0) - (job.investmentTotal || 0)), 0);

  // Active Menu Tab Selection within Character Portal
  const [activeTab, setActiveTab] = useState<'inicio' | 'crafting' | 'purchase' | 'queue' | 'workers' | 'prices' | 'inventory' | 'config'>('inicio');

  // Scroll state for shrinking the estimated profit bar
  const [isScrolled, setIsScrolled] = useState(false);

  // Hover state for expanding the left menu sidebar
  ...
  // Craft Job in Queue
  const handleCraftJob = (jobId: string, keepDiarios?: boolean) => {
    const charQueue = queues[activeCharName] || [];
    const job = charQueue.find(j => j.id === jobId);

    // Deduct usage fee (tasa de uso) from character's capital
    if (job) {
      const fee = job.stationFeeTotal || 0;
      if (fee > 0) {
        setCharacters(prev => prev.map(c => {
          if (c.name === activeCharName) {
            return { ...c, capital: Math.max(0, c.capital - fee) };
          }
          return c;
        }));
      }
    ...
    const updatedQueue = charQueue.map(j => {
      if (j.id === jobId) {
        return { 
          ...j, 
          status: 'crafted_selling' as const,
          useDiarios: keepDiarios !== undefined ? keepDiarios : j.useDiarios
        };
      }
      return j;
    });
    ...
    const finalUseDiarios = keepDiarios !== undefined ? keepDiarios : (job?.useDiarios || false);

    if (job && job.recipe && finalUseDiarios) {
      try {
        const lower = (job.recipe.Artesano || '').toLowerCase();
        let professionId = 'WARRIOR';
        let professionName = 'Diario de Herrero (Lleno)';

        if (lower.includes('mágico') || lower.includes('magico') || lower.includes('imbuidor') || lower.includes('mago')) {
          professionId = 'MAGE';
          professionName = 'Diario de Herrero Mágico (Lleno)';
        } else if (lower.includes('flechero') || lower.includes('cazador') || lower.includes('hunter') || lower.includes('flecha')) {
          professionId = 'HUNTER';
          professionName = 'Diario de Flechero (Lleno)';
        }

        const tierNum = job.tier.replace(/\D/g, '');
        const bookKey = `T${tierNum}_${professionId}`;
        const savedProduced = localStorage.getItem(`albion_produced_books_list_${activeCharName}`);
        const produced = savedProduced ? JSON.parse(savedProduced) : {};
        ...
        const itemName = job.recipe.Item || 'Item Desconocido';
        const diariosToStore = job.diariosQty || 0;
        
        if (diariosToStore > 0) {
          produced[bookKey].items[itemName] = (produced[bookKey].items[itemName] || 0) + diariosToStore;
          produced[bookKey].quantity += diariosToStore;
          localStorage.setItem(`albion_produced_books_list_${activeCharName}`, JSON.stringify(produced));
        }
      } catch (e) {
        console.error("Error storing produced books in localStorage:", e);
      }
    }
  };
    ...
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-200 select-none relative overflow-hidden ${styleConfig.bg}`}>
      ...
      {!activeChar ? (
        <div className="flex-1 flex flex-col justify-center px-4 py-12">
          <div className="max-w-4xl mx-auto w-full">
            ...
            <div className="flex items-center justify-center gap-2 mb-8 text-white">
              ...
              <span className="font-serif font-black text-xl tracking-wider uppercase">VehuiahAlbion</span>
            </div>
        ...
        <>
          ...
          <header className={`border-b sticky top-0 z-40 ${styleConfig.headerBg} ${styleConfig.navBorder} ${isScrolled ? 'py-1' : 'py-2.5'} transition-all duration-300`}>
            <div className={`max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between transition-all duration-305 ${isScrolled ? 'h-10' : 'h-16'}`}>
              ...
              <div className="flex items-center gap-4">
                ...
                {/* Scroll-Responsive Estimated Profit of Active Lots Bar */}
                <div 
                  className={`flex items-center transition-all duration-300 select-none font-mono ${
                    isScrolled 
                      ? 'px-2 py-0.5 bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 text-[10px] rounded-md scale-95' 
                      : 'px-3 py-1.5 bg-emerald-500/5 border border-emerald-500/20 text-emerald-400 text-xs rounded-xl shadow-md'
                  } gap-2`}
                  title="Ganancia neta estimada acumulada de todos tus lotes activos en la cola"
                >
                  <TrendingUp size={isScrolled ? 11 : 14} className="text-emerald-400 animate-pulse" />
                  <span className={isScrolled ? "text-[9px]" : "font-semibold uppercase tracking-wider"}>
                    {isScrolled ? "Ganancia Lotes:" : "Ganancia Estimada Lotes:"}
                  </span>
                  <strong className={`font-mono font-extrabold ${totalActiveProfit >= 0 ? "text-emerald-400 animate-pulse" : "text-red-400"}`}>
                    {totalActiveProfit >= 0 ? '+' : ''}{totalActiveProfit.toLocaleString()} s
                  </strong>
                </div>
          ...
          <div className={
            ...
            <aside 
              ...
              className={
                activeTemplate === 'compact_bento'
                  ? `w-full md:col-span-3 lg:col-span-2 border ${styleConfig.cardBorder} p-4 rounded-xl flex flex-col gap-4 ${styleConfig.panelBg} h-fit shadow-md transition-all duration-300`
                  : activeTemplate === 'split_minimalist'
                  ? 'w-full md:w-56 shrink-0 border-0 p-1 flex flex-col gap-5 transition-all duration-300'
                  : `w-full ${isSidebarHovered ? 'md:w-64' : 'md:w-16'} shrink-0 border-b md:border-b-0 md:border-r p-4 flex flex-col gap-4 ${styleConfig.headerBg} ${styleConfig.navBorder} transition-all duration-300 ease-in-out`
              }
            >
              <span className={`text-[10px] uppercase font-bold text-gray-500 tracking-wider transition-all duration-200 ${isSidebarHovered ? 'md:opacity-100' : 'md:opacity-0 md:h-0 md:overflow-hidden'} hidden md:block`}>
                Categorías
              </span>
              ...
              <div className="flex md:flex-col overflow-x-auto md:overflow-visible gap-2 pb-2 md:pb-0 whitespace-nowrap scrollbar-none font-medium">
                ...
                <button
                  ...
                  className={`py-2.5 px-3 rounded-lg font-semibold flex items-center cursor-pointer transition-all text-xs w-full ${
                    !isSidebarHovered ? 'md:justify-center' : 'md:justify-start gap-2.5'
                  } ${
                    activeTab === 'inicio'
                ...
                >
                  ...
                  <span className={`transition-all duration-250 ${isSidebarHovered ? 'md:inline-block opacity-100' : 'md:hidden opacity-0'}`}>
                    Inicio (Resumen)
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`py-2.5 px-3 rounded-lg font-semibold flex items-center cursor-pointer transition-all text-xs w-full relative ${
                    !isSidebarHovered ? 'md:justify-center' : 'md:justify-start gap-2.5'
                  } ${
                    activeTab === 'purchase'
                      ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30'
                ...
                >
                  ...
                  {activeQueue.filter(j => j.status === 'pending_materials').length > 0 && (
                    <span className={`absolute bg-amber-500 text-black text-[9px] rounded-full font-bold transition-all ${
                      isSidebarHovered ? 'right-2 top-2.5 px-1.5' : 'right-1 top-1 px-1'
                    }`}>
                ...
                >
                  ...
                  {activeQueue.length > 0 && (
                    <span className={`absolute bg-red-500 text-white text-[9px] rounded-full font-bold transition-all ${
                      isSidebarHovered ? 'right-2 top-2.5 px-1.5' : 'right-1 top-1 px-1'
                    }`}>
                      {activeQueue.length}
                    </span>
                ...
                >
                  <CoinsIcon size={15} className="shrink-0" />
                  <span className={`transition-all duration-250 ${isSidebarHovered ? 'md:inline-block opacity-100' : 'md:hidden opacity-0'}`}>
                    Gestión Precios
                  </span>
                </button>
