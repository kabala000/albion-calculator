...
interface InicioPanelProps {
  ...
  setActiveTab: (tab: 'inicio' | 'crafting' | 'purchase' | 'queue' | 'prices' | 'inventory' | 'config') => void;
  primaryColor: string;
  surfaceColor: string;
  borderColor: string;
  ...
  return (
    <div className="max-w-6xl mx-auto py-2 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* CHART BLOCK: Profits Trend */}
        <div className={`lg:col-span-8 p-5 rounded-xl border flex flex-col justify-between ${surfaceColor} ${borderColor} shadow-md`}>
          <div>
            <div className="flex items-center justify-between mb-4 border-b border-gray-800 pb-3">
              <div className="flex items-center gap-2">
                <History className="text-cyan-400" size={16} />
                <strong className="text-sm font-serif text-white tracking-wide">Tendencia de Rendimiento mensual</strong>
              </div>
              <span className="text-[10px] text-gray-400 font-mono">
                {historyLogs.length === 0 ? 'Datos de Ejemplo' : 'Historial Real'}
              </span>
            </div>

            {/* Glowing SVG Bar Chart */}
            <div className="h-44 flex items-end justify-between gap-1 pt-6 font-mono relative mt-1.5">
              <div className="absolute inset-x-0 top-1/2 border-t border-gray-800/40 border-dashed pointer-events-none" />
              <div className="absolute inset-x-0 top-1/4 border-t border-gray-800/20 border-dashed pointer-events-none" />
              <div className="absolute inset-x-0 top-3/4 border-t border-gray-800/20 border-dashed pointer-events-none" />
              
              {chartData.map((d, idx) => {
                const heightPercent = maxVal > 0 ? (Math.abs(d.value) / maxVal) * 80 : 5;
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center group relative z-10">
                    {/* Tooltip on hover */}
                    <div className="absolute -top-10 scale-0 group-hover:scale-100 transition-transform duration-200 bg-slate-950 border border-cyan-500/30 py-1 px-2.5 rounded text-[10px] font-mono text-cyan-400 whitespace-nowrap shadow-xl z-30">
                      {d.value >= 0 ? '+' : ''}{d.value.toLocaleString()} s
                    </div>
                    
                    {/* Bar */}
                    <div className="w-full max-w-[240px] px-1.5">
                      <div 
                        className={`w-full rounded-t-md transition-all duration-500 ease-out shadow-[0_0_10px_rgba(6,182,212,0.15)] group-hover:shadow-[0_0_15px_rgba(34,211,238,0.4)] ${
                          d.value >= 0 
                            ? 'bg-gradient-to-t from-cyan-650/45 to-cyan-400/90 border-t border-cyan-300/30' 
                            : 'bg-gradient-to-t from-red-600/40 to-red-400/80 border-t border-red-300/30'
                        }`}
                    ...
                    {/* Label */}
                    <span className="text-[10px] text-gray-500 mt-2 font-mono group-hover:text-cyan-400 transition-colors">
                      {d.label}
                    </span>
                  </div>
                );
          ...
          <div className="mt-4 pt-3 border-t border-gray-800/50 flex flex-wrap items-center justify-between text-[11px] text-slate-400 gap-3">
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-cyan-400 inline-block" /> Ganancias Netas
            </span>
            <span>
              Total Invertido: <strong className="text-gray-300">{summary.totalInvested.toLocaleString()} s</strong> | Retorno: <strong className="text-cyan-400">{summary.totalRevenue.toLocaleString()} s</strong>
            </span>
          </div>
        </div>
